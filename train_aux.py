...
def train(base_model, mt_classifier, iterator, optimizer, sent_criterion, sar_criterion, multi_task_loss, aux_criterion,scheduler):
    ...
    for data_input, label_input  in Bar(iterator):
        ...
        sarcasm_logits, sentiment_logits, aux_logits = mt_classifier(output, pooled)
        sentiment_probs = nn.Softmax(dim=1)(sentiment_logits).to(device)
        sarcasm_probs = torch.sigmoid(sarcasm_logits).to(device)
        aux_probs = torch.sigmoid(aux_logits).to(device)
        bs = aux_probs.size(0) // 2
        aux_labels = bs * [0] + bs * [1]
        aux_target = torch.tensor(aux_labels, dtype=torch.float).to(device)
        # compute the loss
        ...
        loss_aux = aux_criterion(aux_probs.squeeze(), aux_target)
        ...
        acc_aux += utils.binary_accuracy(aux_probs, aux_target)

    accuracies = {'Sentiment' : acc_sentiment / len(iterator) , 'Sarcasm': acc_sarcasm / len(iterator), 'TaskCls': acc_aux / len(iterator) }
    losses = {'Sentiment' : loss_sent / len(iterator), 'Sarcasm': loss_sarc / len(iterator), 'task cls': loss_tasks / len(iterator) ,'total_loss': mtl_loss / len(iterator)}
    return accuracies, losses

def evaluate(base_model, mt_classifier, iterator, sent_criterion, sar_criterion, multi_task_loss, aux_criterion):
    ...
    with torch.no_grad():
        for data_input, label_input in Bar(iterator):
            ...
            predicted_task = torch.round(aux_probs)
            all_aux_outputs.extend(predicted_task.squeeze().int().cpu().numpy())
            all_aux_targets.extend(aux_target.squeeze().int().cpu().numpy())
    ...
    fscore_task = f1_score(all_aux_outputs, all_aux_targets, average='macro')


    accuracies = {'Sentiment' : acc_sentiment / len(iterator) , 'Sarcasm': acc_sarcasm / len(iterator), 'F1_sentiment' : fscore_sentiment, 'F1_sarcasm': fscore_sarcasm, 'F1_task': fscore_task}
    losses = {'Sentiment' : loss_sent / len(iterator), 'Sarcasm': loss_sarc / len(iterator), 'task cls': loss_tasks / len(iterator) ,
...
def train_full(config, train_loader, stest_loader):
    ...
    mtl_classifier = modeling.MTClassifier4(base_model.output_num()).to(device)
    ...
    for epoch in range(config['epochs']):
        ...
        epo = epoch
        train_accuracies, train_losses = train(base_model, mtl_classifier, train_loader, optimizer, sent_criterion, sarc_criterion, multi_task_loss, aux_criterion,scheduler)
        valid_accuracies, valid_losses = evaluate(base_model, mtl_classifier, valid_loader, sent_criterion, sarc_criterion, multi_task_loss, aux_criterion)
